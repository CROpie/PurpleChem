<script lang="ts">
	...
	const queryDatabase = async (type: string) => {
		...
		if (type == 'string') {
			...
			console.log('searching a structure');
			queryChemicalName = '';
			queryInchi = getInchi();
			if (!queryInchi) {
				searching = false;
...
<div class="mx-8 mt-3">
	...
	<Button type="button" on:click={toggleStructureSearch}>Toggle Structure Search</Button>

	<div bind:this={jsmeContainer} class="hidden">
		<div id="jsme_container" />
		<Button type="button" on:click={() => queryDatabase('structure')}>GO</Button>
	</div>
	{#if searching}
