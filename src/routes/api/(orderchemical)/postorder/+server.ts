...
import { json } from '@sveltejs/kit';
import type { ChemicalInfo, OrderInfo } from '$lib/types/orderChemical.js';

export const POST = async ({ locals, request }) => {
	const APIClient = locals.apiclient;

	const {
		chemicalInfo: chemicalData,
		orderInfo: orderData
	}: { chemicalInfo: ChemicalInfo; orderInfo: OrderInfo } = await request.json();

	const { outcome, data } = await APIClient.post('/order/', null, {
		body: { chemicalData, orderData }
	});
